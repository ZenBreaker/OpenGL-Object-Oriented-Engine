...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if (init_state && g.ActiveId != id)
    {
        ...
        const bool recycle_state = (state->m_ID == id);
        if (recycle_state)
        {
...
static void ImGui::TabBarLayout(ImGuiTabBar* tab_bar)
{
    ...
    if (tab_bar->ReorderRequestTabId != 0)
    {
        if (ImGuiTabItem* tab1 = TabBarFindTabByID(tab_bar, tab_bar->ReorderRequestTabId))
        {
            ...
            if (tab2_order >= 0 && tab2_order < tab_bar->Tabs.Size)
            {
                ...
                *tab1 = *tab2;
                *tab2 = item_tmp;
                if (tab2->m_ID == tab_bar->SelectedTabId)
                    scroll_track_selected_tab_id = tab2->m_ID;
                tab1 = tab2 = NULL;
            }
    ...
    if (tab_list_popup_button)
        if (ImGuiTabItem* tab_to_select = TabBarTabListPopupButton(tab_bar)) // NB: Will alter BarRect.Max.x!
            scroll_track_selected_tab_id = tab_bar->SelectedTabId = tab_to_select->m_ID;
    ...
    for (int tab_n = 0; tab_n < tab_bar->Tabs.Size; tab_n++)
    {
        ImGuiTabItem* tab = &tab_bar->Tabs[tab_n];
        tab->Offset = offset_x;
        if (scroll_track_selected_tab_id == 0 && g.NavJustMovedToId == tab->m_ID)
            scroll_track_selected_tab_id = tab->m_ID;
    ...
    if (tab_bar->SelectedTabId == 0 && tab_bar->NextSelectedTabId == 0 && most_recently_selected_tab != NULL)
        scroll_track_selected_tab_id = tab_bar->SelectedTabId = most_recently_selected_tab->m_ID;
...
{
    if ((tab_bar->VisibleTabId == tab->m_ID) && !(tab->Flags & ImGuiTabItemFlags_UnsavedDocument))
    {
    ...
    else if ((tab_bar->VisibleTabId != tab->m_ID) && (tab->Flags & ImGuiTabItemFlags_UnsavedDocument))
    {
