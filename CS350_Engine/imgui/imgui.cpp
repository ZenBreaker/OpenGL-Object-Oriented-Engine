...
static ImGuiWindow* CreateNewWindow(const char* name, ImVec2 size, ImGuiWindowFlags flags)
{
    ...
    window->Flags = flags;
    g.WindowsById.SetVoidPtr(window->m_ID, window);
    ...
    if (!(flags & ImGuiWindowFlags_NoSavedSettings))
        if (ImGuiWindowSettings* settings = ImGui::FindWindowSettings(window->m_ID))
        {
...
bool ImGui::Begin(const char* name, bool* p_open, ImGuiWindowFlags flags)
{
    ...
    if (window->IDStack.Size == 0)
        window->IDStack.push_back(window->m_ID);
...
{
    ImGuiContext& g = *GImGui;
    return g.NavMoveRequest && g.NavMoveResultLocal.m_ID == 0 && g.NavMoveResultOther.m_ID == 0;
}
...
static void ImGui::NavUpdate()
{
    ...
    if (g.NavMoveRequestForward == ImGuiNavForward_ForwardActive)
    {
        ...
        if (g.NavMoveResultLocal.m_ID == 0 && g.NavMoveResultOther.m_ID == 0)
            g.NavDisableHighlight = false;
...
static void ImGui::NavUpdateMoveResult()
{
    ...
    ImGuiNavMoveResult* result = (g.NavMoveResultLocal.m_ID != 0) ? &g.NavMoveResultLocal : &g.NavMoveResultOther;

    // PageUp/PageDown behavior first jumps to the bottom/top mostly visible item, _otherwise_ use the result from the previous/next page.
    if (g.NavMoveRequestFlags & ImGuiNavMoveFlags_AlsoScoreVisibleSet)
        if (g.NavMoveResultLocalVisibleSet.m_ID != 0 && g.NavMoveResultLocalVisibleSet.m_ID != g.NavId)
            result = &g.NavMoveResultLocalVisibleSet;

    // Maybe entering a flattened child from the outside? In this case solve the tie using the regular scoring rules.
    if (result != &g.NavMoveResultOther && g.NavMoveResultOther.m_ID != 0 && g.NavMoveResultOther.Window->ParentWindow == g.NavWindow)
    ...
    if (g.NavId != result->m_ID)
    {
        // Don't set NavJustMovedToId if just landed on the same spot (which may happen with ImGuiNavMoveFlags_AllowCurrentNavId)
        g.NavJustMovedToId = result->m_ID;
        g.NavJustMovedToMultiSelectScopeId = result->SelectScopeId;
    }
    SetNavIDWithRectRel(result->m_ID, g.NavLayer, result->RectRel);
    g.NavMoveFromClampedRefRect = false;
}
...
ImGuiWindowSettings* ImGui::FindWindowSettings(ImGuiID id)
{
    ImGuiContext& g = *GImGui;
    for (int i = 0; i != g.SettingsWindows.Size; i++)
        if (g.SettingsWindows[i].m_ID == id)
...
static void SettingsHandlerWindow_WriteAll(ImGuiContext* ctx, ImGuiSettingsHandler* handler, ImGuiTextBuffer* buf)
{
    ...
    for (int i = 0; i != g.Windows.Size; i++)
    {
        ...
        ImGuiWindowSettings* settings = (window->SettingsIdx != -1) ? &g.SettingsWindows[window->SettingsIdx] : ImGui::FindWindowSettings(window->m_ID);
        if (!settings)
        {
            settings = ImGui::CreateNewWindowSettings(window->Name);
            window->SettingsIdx = g.SettingsWindows.index_from_ptr(settings);
        }
        IM_ASSERT(settings->m_ID == window->m_ID);
...
void ImGui::ShowMetricsWindow(bool* p_open)
{
    ...
    struct Funcs
    {
        ...
        {
            if (!ImGui::TreeNode((void*)(uintptr_t)columns->m_ID, "Columns Id: 0x%08X, Count: %d, Flags: 0x%04X", columns->m_ID, columns->Count, columns->Flags))
                return;
        ...
        static void NodeTabBar(ImGuiTabBar* tab_bar)
        {
            ...
            if (ImGui::TreeNode(tab_bar, "%s", buf))
            {
                for (int tab_n = 0; tab_n < tab_bar->Tabs.Size; tab_n++)
                {
                    ...
                    ImGui::Text("%02d%c Tab 0x%08X", tab_n, (tab->m_ID == tab_bar->SelectedTabId) ? '*' : ' ', tab->m_ID);
                    ImGui::PopID();
                }
                ImGui::TreePop();
            }
